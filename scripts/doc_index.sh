...
function walk_dir() {
    ...
    # Prevent infinite loop from DEST_DIR
...
if [ -d "$SOURCE_DIR/$FIRST_CHAPTER" ]; then
    echo "📘 Processing $FIRST_CHAPTER directory first..."
    walk_dir "$SOURCE_DIR/$FIRST_CHAPTER" "$FIRST_CHAPTER" ""
fi

# Process other directories in docs/, skipping ignored and FIRST_CHAPTER
